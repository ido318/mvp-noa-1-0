...
export type HandoffDecision = {
  ...
  transfer: boolean;
  /** Whether an escalation should be recorded for Noa to follow up. */
  escalate: boolean;
 ...
 *
 * Policy (per product decision): transfer to Noa's mobile ONLY during business
...
export function decideHumanHandoff(params: {
  ...
  if (within && params.targetNumber) {
    return {
      ...
      result: "מעביר אותך עכשיו לנועה, נא להישאר על הקו.",
    };
  }
  ...
  return {
    ...
    urgency,
    result: within
      ? "רשמתי את הפנייה שלך ונועה תחזור אליך בהקדם."
      : "כרגע מחוץ לשעות הפעילות. רשמתי פנייה דחופה ונועה תחזור אליך בהקדם.",
  };
}
