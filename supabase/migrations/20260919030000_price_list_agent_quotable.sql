-- Separate "what Noa bills" from "what Tomer may say on the phone".
--
-- Neutering costs 350 ₪, and that number is real — Noa charges by it. What is
-- not true is that Tomer can quote it before the visit: the price depends on
-- species, weight, age and medical state, so Noa gives it herself. That is a
-- binding decision (CLAUDE.md) and the frozen SMS wording says
-- 'המחיר יימסר על ידי ד"ר נועה'.
--
...
comment on column public.price_list_items.agent_quotable is
  'False when Tomer must not name this price before the visit, because it depends on the individual animal. Billing still uses default_price; the pre-visit SMS says המחיר יימסר על ידי ד"ר נועה instead.';
