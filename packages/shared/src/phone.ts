...
// a number Noa can call a client back on.
 ...
 *   "054-958-1991"    → "549581991"   (local, with separators)
 *   "+972 54 9581991" → "549581991"   (E.164, with separators)
 *   "972549581991"    → "549581991"   (E.164 without the +)
 *   "549581991"       → "549581991"   (leading 0 dropped — common in forms
...
function toNsn(raw: string): string | null {
  ...
  if (digits.startsWith("972") && digits.length > 9) {
    ...
    // "+9720549581991" — some systems keep the trunk 0 after the country
...
/**
 * Format an E.164 Israeli number the way Israelis read it: "+972549581991"
 * → "054-958-1991". Falls back to the input unchanged when it is not a
