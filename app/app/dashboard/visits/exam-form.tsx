...
export function ExamForm({ visitId }: { visitId: string }) {
  ...
  return (
    <form id="physical-exam" onSubmit={onSave} className="space-y-3">
            ...
            <div key={system} className="rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--surface-2)] p-3">
              ...
              {state.status === "abnormal" ? (
                <label className="mt-2 block space-y-1">
                  <span className="text-[11px] font-semibold text-[var(--muted)]">תיאור הממצא</span>
                  <textarea
                    value={state.note}
                    onChange={(event) => setNote(system, event.target.value)}
                    rows={2}
                    className="w-full rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--bg)] px-2 py-1.5 text-xs text-[var(--ink)] outline-none focus:border-[var(--brand-400)]"
                  />
                </label>
              ) : null}
            </div>
